...
function invertirLista(listaDeNumeros) {
    listaDeNumeros= listaDeNumeros.reverse(listaDeNumeros);
...
function calcularPromedio(listaDeNumeros) {
     let suma=0;
     let total=0;
     let prom=listaDeNumeros.length;
   for(let i=0; i<listaDeNumeros.length ;i++){
    suma += listaDeNumeros[i];
    total=suma/prom;
   }
...
console.log("crearListaDeNumeros(2,5): ", crearListaDeNumeros(2,5)); // [2, 3, 4, 5]
...
function enumerarLista(listaDePalabras) {
  ...
  const frase = `${todasMenosUltima} y ${ultima}.`;
